...
public class ThongkethuocNhapJPanel extends JPanel implements ActionListener{
	...
	public ThongkethuocNhapJPanel() {
		...
		srcBangthongke.setBorder(BorderFactory.createTitledBorder("Danh sách thuốc hết hạn"));
		...
		List<Thuoc> dsthuoc = tkThuocNhap_DAO.thongKethuocnhap();
		for(Thuoc t : dsthuoc) {
			int tong = (int) t.getDonGia();
			dataset.addValue(tong, "Thuốc nhập", t.getNgayNhap());
		}
		JFreeChart chart = ChartFactory.createBarChart("Thuốc nhập", "Ngày nhập", "Số lượng", dataset);
